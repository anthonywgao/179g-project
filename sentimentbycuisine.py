...
avgRatingByCuisine.filter(col("categories").contains("Restaurant") & (col("categories").contains("Chinese") | col("categories").contains("Thai") | col("categories").contains("Japanese") | col("categories").contains("Korean") | col("categories").contains("Indian") | col("categories").contains("American") | col("categories").contains("Caribbean") | col(
    "categories").contains("Italian") | col("categories").contains("Mediterranean") | col("categories").contains("Mexican") | col("categories").contains("Vietnamese") | col("categories").contains("Cajun") | col("categories").contains("Greek"))).withColumn("categories", udf_pfc(col("categories"))).groupBy("categories").agg({'predicted_sentiment': 'avg', }).show(truncate=False)
...
avgRatingByCuisine.write \
    ...
    .option("collection", "avg_sentiment_cuisine") \
    .save()
